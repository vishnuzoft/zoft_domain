...
import { CartItem, CartResponse, GetCartItemByIdResponse, OrderItem, OrderResponse } from "../models";
...
class CartRepository {
    ...
    static async createOrderItem(client: PoolClient, orderItem: OrderItem,user_id:string): Promise<OrderResponse> {
        try {
            const result=await client.query(
                "INSERT INTO tbl_domain_order_items (user_id,domain, duration, price) VALUES ($1, $2, $3, $4) RETURNING order_item_id, user_id, domain, duration, price",
                [user_id,orderItem.domain, orderItem.duration, orderItem.price]
            );
